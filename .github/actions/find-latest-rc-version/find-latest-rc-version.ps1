...
.EXAMPLE
    .\find-latest-rc-version.ps1 -GitHubToken $token -Repository "optivem/optivem-testing-dotnet"
    .\find-latest-rc-version.ps1 -RcVersion "2026.02.05-rc.47" -GitHubToken $token -Repository "optivem/optivem-testing-dotnet"
#>
...
if ([string]::IsNullOrWhiteSpace($RcVersion)) {
    ...
    try {
        # Use correct GitHub Packages API URL for NuGet
        $response = Invoke-RestMethod -Uri "https://api.github.com/orgs/$owner/packages/nuget/$PackageName/versions" -Headers $headers
        ...
        Write-Host "❌ Failed to fetch RC versions from GitHub Packages: $_" -ForegroundColor Red
        exit 1
    }
} else {
